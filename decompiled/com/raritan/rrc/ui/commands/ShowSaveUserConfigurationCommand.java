...
public class ShowSaveUserConfigurationCommand
extends AbstractCommand {
    public static final String COMMAND_KEY = "showSaveUserConfigurationCommand";

    public ShowSaveUserConfigurationCommand(ScreenContext screenContext) {
        super(screenContext);
    }
    ...
    public boolean isExecutable() {
        ...
        if (arrayList != null && arrayList.size() > 0 && (device = (Device)arrayList.get(0)) != null && device instanceof IPReach && device.isConnected() && ((IPReach)device).isAdministrator()) {
            return device.getHandler().isShowSaveUserConfigurationExecutable();
        }
        return false;
    }
}
