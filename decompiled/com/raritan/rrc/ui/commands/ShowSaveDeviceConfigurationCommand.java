...
public class ShowSaveDeviceConfigurationCommand
extends AbstractCommand {
    public static final String COMMAND_KEY = "showSaveDeviceConfigurationCommand";

    public ShowSaveDeviceConfigurationCommand(RRCScreenContext rRCScreenContext) {
    ...
    public boolean isExecutable() {
        ...
        if (arrayList != null && arrayList.size() > 0 && (device = (Device)arrayList.get(0)) != null && device instanceof IPReach && device.isConnected() && ((IPReach)device).isAdministrator()) {
            return device.getHandler().isShowSaveDeviceConfigurationExecutable();
        }
        return false;
    }
}
