...
public class DoRefreshScreenCommand
extends AbstractCommand {
    public static final String COMMAND_KEY = "doRefreshScreenCommand";

    public DoRefreshScreenCommand(ScreenContext screenContext) {
        super(screenContext);
    }
    ...
    public CommandResult execute() {
        ...
        if (port != null && port instanceof KvmPort && port.isConnected()) {
            DeviceView deviceView = port.getView();
            if (deviceView != null) {
                deviceView.getCommandHandler().handleCommand(this, this.scrContext);
            } else {
                RRCLogger.log(100, 1, "View is null in DoRefreshScreenCommand");
            }
        }
