...
public class ShowVirtualMediaLocalPanelCommand
extends AbstractCommand {
    public static final String COMMAND_KEY = "showVirtualMediaLocalPanelCommand";
    private String tooltip = null;

    public ShowVirtualMediaLocalPanelCommand(ScreenContext screenContext) {
        super(screenContext);
    }
    ...
    public boolean isExecutable() {
        ...
        if (!Platform.isVirtualMediaSupported()) {
            this.tooltip = "<html>" + this.getBundle().getString("vm.tooltip.notsupported").replaceAll("\n", "<br>") + "</html>";
            return false;
        }
        ...
        if (vMConfigInfo != null && vMConfigInfo.isCimActive() && (vMInterfaceInfo = vMConfigInfo.getVMInterface(4)) != null) {
            if (vMInterfaceInfo.isMultiLUN() && vMInterfaceInfo.isInUse() && port.isTopLeft()) {
                MassStorageDevice massStorageDevice = vMInterfaceInfo.getMassStorageDevice(4);
