...
public class ShowSaveDiagnosticLogCommand
extends AbstractCommand {
    public static final String COMMAND_KEY = "showSaveDiagnosticLogCommand";

    public ShowSaveDiagnosticLogCommand(RRCScreenContext rRCScreenContext) {
    ...
    public boolean isExecutable() {
        ...
        if (arrayList.size() > 0 && (device = (Device)arrayList.get(0)) != null && device instanceof IPReach && device.isConnected() && ((IPReach)device).isAdministrator()) {
            return device.getHandler().isShowSaveDiagnosticLogExecutable();
        }
        return false;
    }
}
