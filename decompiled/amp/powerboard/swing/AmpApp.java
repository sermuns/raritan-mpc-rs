...
PowerboardInterface {
    ...
    public void init() {
        ...
        String platformVal = this.getParameter("Platform");
        if (platformVal != null && !platformVal.equalsIgnoreCase("JavaRRC")) {
            this.initialize();
        } else {
            this.platform = "JavaRRC";
            this.stateManager1.setPlatform(this.platform);
            this.stateManager1.setPowerboardInterface(this);
        }
        ...
        if (sLoginName != null && sPassword != null) {
            try {
                ...
                if (this.platform.equals("COMMAND_CENTER")) {
                    JOptionPane.showMessageDialog(this.stateManager1.getFrame(), "The private account of " + this.deviceName + " is in use. Please check with your administrator ", "Error", 0);
                } else {
                    try {
                ...
                JOptionPane.showMessageDialog(this.stateManager1.getFrame(), "Console Port Access", "Maximum Users Exceeded", 0);
            }
    ...
    public void addStateManager() {
        this.getContentPane().add(this.stateManager1.getPowerboardUI());
    }

    public void setSerialStream(ISerialStream serialStream) {
        this.stateManager1.setSerialStream(serialStream);
    }

    public void setDataBytes(byte[] data) {
        this.stateManager1.bytesArrived(data, data.length);
    }
    ...
    public void commandDataArrived(CDataEvent dataEvent) {
        switch (dataEvent.getOpcode()) {
            ...
            case 17: {
                JOptionPane.showMessageDialog(this.stateManager1.getFrame(), "You have been logged out.", "Forced Logout", 0);
