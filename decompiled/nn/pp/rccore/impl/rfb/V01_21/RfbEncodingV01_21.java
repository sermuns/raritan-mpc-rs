...
import nn.pp.rccore.impl.rfb.V01_21.Converter;

public class RfbEncodingV01_21
extends RfbEncoding {
    ...
    private static RCCore.ColorDepth getColorDepthFromString(String string) {
        RfbEncodingV01_21.loadConverter();
    ...
    public RfbEncodingV01_21(Logger logger, VideoEventListenerList videoEventListenerList) {
    ...
    private void addSoftwareEncodings() {
        ...
        this.compressionsSw.add(new Compression(RCCore.Compression.VIDEO_OPTIMIZED, vector2, 10, 0, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.UNCOMPRESSED, vector2, 5, 0, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_1, vector, 7, 1, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_2, vector, 7, 2, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_3, vector, 7, 3, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_4, vector, 7, 4, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_5, vector, 7, 5, false));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_6, vector, 7, 6, true));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_7, vector, 7, 7, true));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_8, vector, 7, 8, true));
        this.compressionsSw.add(new Compression(RCCore.Compression.LEVEL_9, vector, 7, 9, true));
    ...
    public void setEncodingPredefineFromRfb(String string) {
        if (this.encodingType == 2) {
            ...
            this.setPredefine(RfbEncodingV01_21.getPredefineFromString(string));
    ...
    public void setEncodingCompressionFromRfb(String string) {
        this.setEncodingCompression(RfbEncodingV01_21.getCompressionFromString(string));
    }
    ...
    public void setEncodingColorDepthFromRfb(String string) {
        this.setEncodingColorDepth(RfbEncodingV01_21.getColorDepthFromString(string));
    }
    ...
    public int[] getRfbEncodings() {
        ...
        if (this.encodingType == 1) {
            int n2;
            Compression compression2 = this.lookupCompression(RCCore.Compression.LEVEL_5);
            if (this.supportHwEnc) {
                n2 = 128;
                n2 |= colorDepth.getRfbLrleSubencLossless() << 12;
                n2 |= compression2.rfbCompressionLevel << 8;
                nArray[n++] = 255;
            } else {
                n2 = compression2.rfbEncoding;
                n2 |= colorDepth.getSubformat() << 12;
                n2 |= compression2.rfbCompressionLevel << 8;
                nArray[n++] = 127;
            }
            nArray[n++] = n2;
        } else if (this.supportHwEnc) {
            int n3 = 128;
            n3 = this.lossy ? (n3 |= colorDepth.getRfbLrleSubencLossy() << 12) : (n3 |= colorDepth.getRfbLrleSubencLossless() << 12);
            nArray[n++] = n3 |= compression.rfbCompressionLevel << 8;
        } else {
            int n4 = compression.rfbEncoding;
            n4 |= colorDepth.getSubformat() << 12;
            n4 |= compression.rfbCompressionLevel << 8;
            if (compression.getComp() == RCCore.Compression.VIDEO_OPTIMIZED) {
                n4 |= 0x20000;
            }
            nArray[n++] = n4;
            if (compression.isCacheUsed()) {
                nArray[n++] = 9;
            }
        }
        return nArray;
    ...
    class ColorDepth {
        ...
        public int getRfbLrleSubencLossless() {
            return this.rfbLrleSubencLossless;
        }

        public int getRfbLrleSubencLossy() {
            return this.rfbLrleSubencLossy;
        }

        public int getSubformat() {
            return this.rfbSubencSw;
        }
    }
    ...
    class Compression {
        ...
        Compression(RCCore.Compression compression, List<RCCore.ColorDepth> list, int n, int n2, boolean bl) {
            this(compression, list, n, n2);
            this.useCache = bl;
        }

        public RCCore.Compression getComp() {
            return this.comp;
        }
